<#
.SYNOPSIS
  Andrews-Super-Content-Downloader GUI for Manifest, Content & Report Template Upload
 
.DESCRIPTION
  A simple GUI wrapper for AZCopy.exe to simplify the process of Downloading Content to Local Storage
...
.EXAMPLE
  .\Andrews-Super-Content-Downloader.ps1
#>
...
$form.Text = "Andrews-Super-Content-Downloader.ps1"
...
[pscustomobject]@{Name='PRODUCT FOLDER 1';Value="\\TFSBUILDSERVER.DOMAIN.com\C$\t\Hyve\23\b"}
[pscustomobject]@{Name='PRODUCT FOLDER 2';Value="\\TFSBUILDSERVER.DOMAIN.com\C$\t\Hyve\15\b"}
[pscustomobject]@{Name='PRODUCT FOLDER 3';Value="\\TFSBUILDSERVER.DOMAIN.com\C$\t\Hyve\2\b"}
)
...
$label4.Size = '248, 23'
$label4.TabIndex = 10
$label4.Text = "Andrews Super Content Downloader"
...
Get-ChildItem -Path $Target -Filter *.* | % { $_.BaseName } | Select-Object -Unique | ForEach-Object { New-Item -Name $_ -Path $Target -ItemType directory  } | Out-Null
cd $Target
Get-ChildItem -Path $Target -Filter *.zip | % { $_.BaseName -match "([a-zA-Z0-9-]+)"  | Out-Null; Move-Item $_ -Destination ".\$($Matches[1])\$_" } | Out-Null
Get-ChildItem -Path $Target -Filter *.html | % { $_.BaseName -match "([a-zA-Z0-9-]+)" | Out-Null; Move-Item $_ -Destination ".\$($Matches[1])\$_" } | Out-Null